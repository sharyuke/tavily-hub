...
> **付费使用注册 Tavily 需要海外信用卡，门槛高、续费难，免费额度1000，又太少，注册多个账号，管理不方便，需要在多个key之间切换。**
...
- ✅ 自动注册三个工具：`tavily_search`、`tavily_extract`、`tavily_crawl`（`/crawl` · 站点爬取）
...
接入成功后，所有客户端都能看到三个工具：`tavily_search`、`tavily_extract`、`tavily_crawl`（`/crawl` · 站点爬取）。
...
- ✅ **MCP 同样生效** — 通过 `/api/mcp` 或 `/api/mcp/sse` 调 `tavily_search` / `tavily_extract` / `tavily_crawl` 时也走自费通道
